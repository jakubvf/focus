...
pub const ProjectFileOpener = struct {
    ...
    pub fn init(app: *App) *ProjectFileOpener {
        ...
        {
            const file = std.fs.cwd().openFile("C:\\Users\\jvf\\Desktop", .{}) catch |err|
                u.panic("{} while opening {s}", .{ err, "C:\\Users\\jvf\\Desktop" });
            defer file.close();
            file.reader().readAllArrayList(&projects, std.math.maxInt(usize)) catch |err|
                u.panic("{} while reading {s}", .{ err, "C:\\Users\\jvf\\Desktop" });
        }
